...
exports.run = async(client, message, args) => {
    ...
    if (taglıalım) {
        if (!member.user.username.includes(ayar.tag) && !member.roles.cache.has(ayar.boosterRolü) && !member.roles.cache.has(ayar.vipRolü)) {
            return message.channel.send(embed.setDescription(`${member}**, adlı kullanıcının isminde tag bulunmadığı için ve vip yada booster üye olmadığı için kayıt gerçekleştirilemedi!**`))
        }
    }
    ...
    message.channel.send(embed.setDescription(`<a:vlrn_onay:833959884785582092> **[1/2]** ${member} **kullanıcısının ismi** \`${name}\` **olarak değiştirildi.**\n\n<a:rdmnc_sag_ok:832605528482906112> **Kullanıcının cinsiyetini tepkiye tıklayarak seçin.**`)).then(async msg => {
        ...
        let erkek = msg.createReactionCollector(erkekemoji, { time: 7000, max: 1 })
        let kız = msg.createReactionCollector(kadinemoji, { time: 7000, max: 1 })
        let isimxd = msg.createReactionCollector(isimemoji, { time: 7000, max: 1 })
        ...
        erkek.on("collect", async() => {
            ...
            await msg.edit(embed.setDescription(`<a:vlrn_onay:833959884785582092> **[2/2]** ${member} **kullanıcısının cinsiyeti** \`erkek\` **olarak ayarlandı.**\n\n<a:rdmnc_sag_ok:832605528482906112> **Kayıt başarılı!**`)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
        ...
        kız.on("collect", async() => {
            ...
            await msg.edit(embed.setDescription(`<a:vlrn_onay:833959884785582092> **[2/2]** ${member} **kullanıcısının cinsiyeti** \`kadın\` **olarak ayarlandı.**\n\n<a:rdmnc_sag_ok:832605528482906112> **Kayıt başarılı!**`)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
