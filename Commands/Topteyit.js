...
exports.run = async(client, message, args) => {
    ...
    let topteyit = x.filter(dat => message.guild.members.cache.has(dat)).sort((a, b) => Number((data[b].erkek || 0) + (data[b].kız || 0)) - Number((data[a].erkek || 0) + (data[a].kız || 0))).map((value, index) => `\`${index + 1}.\` ${message.guild.members.cache.get(value)} **Toplam** \`${((data[value].erkek || 0) + (data[value].kız || 0))}\` **(**\`${((data[value].erkek || 0))}\` **Erkek** \`${((data[value].kız || 0))}\` **Kadın)**`).splice(0, 25);
    message.channel.send(embed.setDescription(topteyit || '**Sunucuya ait kayıt verisi bulunamadı.**'))
}
exports.conf = {
    enabled: true,
    guildOnly: true,
    aliases: ['top-kayıt','top-teyit'],
    name: 'topteyit',
    permLevel: 0
};